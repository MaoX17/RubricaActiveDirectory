...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.index', ['servizi' => $servizi, 'aree' => $aree, 'results' => $results]);
    ...
    //    echo "Area = ".$area."<br>";
        ...
        */
        ...
        $results = Adldap::search()->users()
            ->whereHas('title')
            ->where('cn', 'contains', $search_cognome)
            ->where('company', $search_area)
            ->where('department', $search_servizio)
            ->sortBy('cn', 'asc')
            ->get();
*/
...
/*
        ...
        if ($cognome == "all") {
            if ($servizio == "all") {
                if ($area == "all") {
                    $results = Adldap::search()->users()->whereHas('title')->sortBy('cn', 'asc')->get();
                }
                else {
                    $results = Adldap::search()->users()->where('title', '*')->where('company', '=', $area)->sortBy('cn', 'asc')->get();
                }
            }
            else {
                if ($area == "all") {
                    $results = Adldap::search()->users()->where('title', '*')->where('physicaldeliveryoffice', '=', $servizio)->sortBy('cn', 'asc')->get();
                } else {
                    $wheres = [
                        'title' => '*',
                        'company' => $area,
                        'physicaldeliveryofficename' => $servizio,
                    ];
                    $results = Adldap::search()->users()->where($wheres)->sortBy('cn', 'asc')->get();
                }
            }
        }
        elseif (strlen($cognome) == 1) {

            $results = Adldap::search()->users()->where('title', '*')->where('cn', 'starts_with', $cognome)->sortBy('cn', 'asc')->get();
        }
        else {
            if ($servizio == "all") {
                if ($area == "all") {
                    $results = Adldap::search()->users()->where('cn', 'contains', $cognome)->sortBy('cn', 'asc')->get();
                } else {
                    $results = Adldap::search()->users()->where('cn', 'contains', $cognome)->where('company', '=', $area)->sortBy('cn', 'asc')->get();
                }
            } else {
                if ($area == "all") {
                    $results = Adldap::search()->users()->where('cn', 'contains', $cognome)->sortBy('cn', 'asc')->get();
...
END:VCARD");
        ...
        return view('frontend.index', ['servizi' => $servizi, 'aree' => $aree, 'results' => $obj_contact]);
    }
...
//            'cn' => 'John Doe',
  //      ]);
        ...
        return view('frontend.test', ['servizi' => $servizi, 'aree' => $aree]);
    }
